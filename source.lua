if IY_LOADED and not _G.IY_DEBUG then
	-- error("Limited Yield is already running!", 0)
	return
end
...
Title.TextSize = 18
	Title.Text = "Limited Yield v" .. currentVersion
...
PluginsHint.TextSize = 16
	PluginsHint.Text = "Drop .lua plugins in the workspace folder"
...
reference = (function()
	local main = create({
		...
		{114,"TextLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Font=3,Name="Text",Parent={112},Position=UDim2.new(0,8,0,30),Size=UDim2.new(1,-8,0,32),Text="Limited Yield. Built clean. Run your own plugins via the Plugins folder.",TextColor3=Color3.new(1,1,1),TextSize=14,TextWrapped=true,TextXAlignment=0,ZIndex=10,}},
		...
		{116,"TextButton",{BackgroundColor3=Color3.new(0.18,0.18,0.18),BorderColor3=Color3.new(0.18,0.18,0.18),Font=4,Name="InviteButton",Parent={112},Position=UDim2.new(0,5,0,75),Size=UDim2.new(1,-10,0,25),Text="Limited Yield",TextColor3=Color3.new(1,1,1),TextSize=16,ZIndex=10,}},
	})
...
function CreateJoinLabel(plr,ID)
	...
	infoFrame.BorderColor3 = Color3.new(0.18, 0.18, 0.18)
...
SaveChatlogs.MouseButton1Down:Connect(function()
	if writefileExploit() then
		if #scroll_2:GetChildren() > 0 then
			...
			local writelogs = '-- Limited Yield Chat logs for "'..placeName..'"\n'
			for _, child in pairs(scroll_2:GetChildren()) do
				writelogs = writelogs..'\n'..child.Text
			end
...
CMDs = {}
CMDs[#CMDs + 1] = {NAME = 'discord / support / help', DESC = 'Limited Yield info'}
...
CMDs[#CMDs + 1] = {NAME = 'hideiy', DESC = 'Hides the main LY GUI'}
CMDs[#CMDs + 1] = {NAME = 'showiy / unhideiy', DESC = 'Shows LY again'}
CMDs[#CMDs + 1] = {NAME = 'keepiy', DESC = 'Auto execute LY when you teleport through servers'}
...
CMDs[#CMDs + 1] = {NAME = '', DESC = ''}
CMDs[#CMDs + 1] = {NAME = 'soundlist', DESC = 'Lists all Sound instances in the game with their names and IDs'}
CMDs[#CMDs + 1] = {NAME = 'playsound [id]', DESC = 'Creates and plays a Sound with the given asset ID'}
CMDs[#CMDs + 1] = {NAME = 'stopsound', DESC = 'Stops and removes all sounds created by playsound'}
CMDs[#CMDs + 1] = {NAME = 'freecamtp', DESC = 'Teleports you to where your freecam is currently looking'}
CMDs[#CMDs + 1] = {NAME = 'konstant', DESC = 'Loads the konstant script'}
-- wait()
...
Players.LocalPlayer.OnTeleport:Connect(function(State)
	if KeepInfYield and (not TeleportCheck) and queueteleport then
		TeleportCheck = true
		queueteleport("loadstring(game:HttpGet('https://raw.githubusercontent.com/lowkeymyself/Limited-Yield/main/source.lua'))()")
	end
end)
...
addcmd('discord', {'support', 'help'}, function(args, speaker)
	notify('Limited Yield', 'No Discord. Standalone build.')
end)

addcmd('keepiy', {}, function(args, speaker)
	if queueteleport then
		KeepInfYield = true
		notify('KeepIY','Limited Yield will now run after you teleport')
		updatesaves()
	else
...
addcmd('unkeepiy', {}, function(args, speaker)
	if queueteleport then
		KeepInfYield = false
		notify('KeepIY','Limited Yield will no longer run after you teleport')
		updatesaves()
	else
		notify('Incompatible Exploit','Your exploit does not support this command (missing queue_on_teleport)')
	end
...
local lySounds = {}
addcmd('soundlist', {}, function(args, speaker)
	local count = 0
	for _, v in pairs(workspace:GetDescendants()) do
		if v:IsA("Sound") then
			count = count + 1
			notify("Sound #"..count, tostring(v.Name).." | ID: "..tostring(v.SoundId))
		end
	end
	notify("soundlist", "Found "..count.." sounds. Check console for full list.")
	for _, v in pairs(workspace:GetDescendants()) do
		if v:IsA("Sound") then
			print("[LY soundlist] Name:", v.Name, "| ID:", v.SoundId, "| Parent:", v.Parent:GetFullName())
		end
	end
end)

addcmd('playsound', {}, function(args, speaker)
	local id = args[1]
	if not id then notify("playsound", "Provide a sound ID") return end
	if not isNumber(id) then notify("playsound", "ID must be a number") return end
	...
	s.Parent = workspace
	s:Play()
	table.insert(lySounds, s)
	notify("playsound", "Playing sound "..id)
end)

addcmd('stopsound', {}, function(args, speaker)
	local n = 0
	for i = #lySounds, 1, -1 do
		local s = lySounds[i]
		if s then
			s:Stop()
			s:Destroy()
		...
		table.remove(lySounds, i)
	end
	notify("stopsound", "Stopped and removed "..n.." sounds")
end)

addcmd('freecamtp', {}, function(args, speaker)
	if not fcRunning then notify("freecamtp", "Freecam is not active. Start it with ;freecam") return end
	local char = speaker.Character
	local root = char and getRoot(char)
	local cam = workspace.CurrentCamera
	if root and cam then
		local target = cam.CFrame.Position + cam.CFrame.LookVector * 5
		root.CFrame = CFrame.new(target)
		notify("freecamtp", "Teleported to camera focus")
	end
end)

addcmd('konstant', {}, function(args, speaker)
	local ok, err = pcall(function()
		loadstring(game:HttpGet("https://raw.githubusercontent.com/lowkeymyself/HowToAccountJadeShipClaudeTryHackMe/refs/heads/main/tes.lua"))()
	end)
	if not ok then
		notify("konstant", "Failed to load: "..tostring(err))
	else
		notify("konstant", "Loaded")
	end
end)
...
task.spawn(function()
	local success, latestVersionInfo = pcall(function() 
		local versionJson = game:HttpGet("https://raw.githubusercontent.com/lowkeymyself/Limited-Yield/main/version")
		return HttpService:JSONDecode(versionJson)
	end)

	if success then
		if currentVersion ~= latestVersionInfo.Version then
			notify("Outdated", "Get the new version at github.com/lowkeymyself/Limited-Yield")
		end
